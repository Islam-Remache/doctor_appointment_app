...
import com.example.prescription_manag2.ui.screens.patient.PrescriptionListScreen
import com.example.prescription_manag2.viewmodel.PrescriptionViewModel


@RequiresApi(Build.VERSION_CODES.O)
@Composable
fun PrescriptionListApp(navController: NavController) {
    val context = LocalContext.current
    val application = context.applicationContext as MyApplication
    val prescriptionViewModel = PrescriptionViewModel(application.prescriptionRepository)
    PrescriptionListScreen(prescriptionViewModel, context, navController)
}